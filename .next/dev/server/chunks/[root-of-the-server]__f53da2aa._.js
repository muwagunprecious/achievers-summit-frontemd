...
async function generateTicketPDF({ fullName, ticketType, ticketId }) {
    return new Promise(async (resolve, reject)=>{
        try {
            console.log(`[PDF-SERVICE] Generating PDF for ${fullName} (${ticketId}) using standard fonts...`);
            ...
            // --- Brand Header ---
            doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(18).text('ACHIEVERS SUMMIT', 20, 30, {
                characterSpacing: 2
            });
            doc.fillColor('#ff8c00').fontSize(18).text('2026', 220, 30, {
                italic: true
            });
            // --- Attendee Info ---
            doc.fillColor('#ffffff').fontSize(10).font('Helvetica').text('OFFICIAL EVENT TICKET', 20, 65, {
                characterSpacing: 1
            });
            doc.rect(20, 80, doc.page.width - 40, 1).fill('#ffffff');
            doc.fillColor('#ff8c00').fontSize(10).text('ATTENDEE', 20, 100);
            doc.fillColor('#ffffff').fontSize(16).font('Helvetica-Bold').text(fullName.toUpperCase(), 20, 115);
            doc.fillColor('#ff8c00').fontSize(10).font('Helvetica').text('CATEGORY', 20, 150);
            doc.fillColor('#ffffff').fontSize(14).font('Helvetica-Bold').text(ticketType.toUpperCase(), 20, 165);
            doc.fillColor('#ff8c00').fontSize(10).font('Helvetica').text('TICKET ID', 20, 200);
            doc.fillColor('#ffffff').fontSize(14).font('Helvetica-Bold').text(ticketId, 20, 215);
            // --- QR Code ---
            ...
            doc.image(qrImageBuffer, 220, 150, {
                width: 100
            });
            // --- Footer / Venue ---
            doc.rect(0, doc.page.height - 60, doc.page.width, 60).fill('#111111');
            doc.fillColor('#666666').fontSize(8).text('VENUE', 20, 370);
            doc.fillColor('#ffffff').fontSize(10).text('Victoria Island, Lagos', 20, 385);
            doc.fillColor('#666666').fontSize(8).text('DATE', 220, 370);
            doc.fillColor('#ffffff').fontSize(10).text('31 March - 01 April', 220, 385);
            doc.end();
        } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__f53da2aa._.js.map