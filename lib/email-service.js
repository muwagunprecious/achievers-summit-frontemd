...
// Verify connection configuration only when needed, not at top-level build time
const verifyConnection = async () => {
    try {
        if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
            console.warn('[EMAIL-SERVICE] SMTP credentials not found, skipping verification.');
            return false;
        }
