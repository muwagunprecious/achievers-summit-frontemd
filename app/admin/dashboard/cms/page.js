    ...
    const renderFAQTab = () => (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => addItem('faq')} className="btn btn-primary px-6 py-2 rounded-xs flex items-center gap-2 text-xs font-black tracking-widest transition-all active:scale-95">
                    <Plus size={16} /> ADD QUESTION
                </button>
            </div>
            <div className="space-y-4">
                {faqs.map((faq, idx) => (
                    <div key={faq.id || `new-${idx}`} className="glass-panel p-8 rounded-xs border border-white/5 hover:border-primary-copper/20 transition-all duration-500 group">
                        <div className="flex gap-6 items-start">
                            <div className="flex-1 space-y-4">
                                <input
                                    className="w-full bg-white/5 border border-white/10 rounded-xs px-6 py-4 text-white outline-none focus:border-primary-copper/50 transition-all font-bold placeholder:text-white/20"
                                ...
                                />
                                <textarea
                                    className="w-full h-24 bg-white/5 border border-white/10 rounded-xs px-6 py-4 text-white outline-none focus:border-primary-copper/50 transition-all resize-none placeholder:text-white/20 leading-relaxed"
                                    placeholder="Answer"
                                    value={faq.answer}
                                    onChange={(e) => {
                                        const newList = [...faqs];
                            ...
                            <div className="flex flex-col gap-3">
                                <button onClick={() => handleSave('faq', faq)} className="w-12 h-12 rounded-xs bg-green-500/10 text-green-500 flex items-center justify-center hover:bg-green-500 hover:text-white transition-all">
                                    <Save size={18} />
                                </button>
                                <button onClick={() => handleDelete('faq', faq.id)} className="w-12 h-12 rounded-xs bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">
                                    <Trash2 size={18} />
                                </button>
                            </div>
                        </div>
                ...
                {['PRODUCT', 'SERVICE'].map(section => (
                    <div key={section} className="space-y-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => addItem('unit', { section })} className="w-8 h-8 rounded-xs bg-primary-copper/10 text-primary-copper flex items-center justify-center hover:bg-primary-copper hover:text-white transition-all">
                                <Plus size={16} />
                            </button>
                        </div>
                        <div className="space-y-4">
                            {units.filter(u => u.section === section).map((unit, idx) => (
                                <div key={unit.id || `new-${section}-${idx}`} className="flex gap-3 group">
                                    <input
                                        className="flex-1 bg-white/5 border border-white/10 rounded-xs px-6 py-3 text-xs text-white outline-none focus:border-primary-gold/50 transition-all"
                                        value={unit.name}
                                        onChange={(e) => {
                                            const newList = [...units];
    ...
    const renderInstitutionsTab = () => (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => addItem('institution')} className="btn btn-primary px-6 py-2 rounded-xs flex items-center gap-2 text-xs font-black tracking-widest transition-all active:scale-95">
    ...
    const renderStatusesTab = () => (
        <div className="space-y-6">
            <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => addItem('status')} className="btn btn-primary px-6 py-2 rounded-xs flex items-center gap-2 text-xs font-black tracking-widest transition-all active:scale-95">
                    <Plus size={16} /> ADD ROLE
                </button>
            </div>
            <div className="grid grid-cols-1 gap-6">
                {statuses.map((stat, idx) => (
                    <div key={stat.id || `new-status-${idx}`} className="glass-panel p-8 rounded-xs border border-white/5 flex flex-col md:flex-row gap-6 items-center">
                        <div className="w-16 h-16 rounded-xs bg-primary-copper/10 text-primary-copper flex items-center justify-center">
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                            <div className="space-y-2">
                                ...
                                <input
                                    className="w-full bg-white/5 border border-white/10 rounded-xs px-6 py-3 text-sm text-white outline-none focus:border-primary-copper/50"
                                    value={stat.label}
                                    onChange={(e) => {
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    className="w-full h-[52px] bg-white/5 border border-white/10 rounded-xs px-6 text-sm text-white outline-none focus:border-primary-copper/50"
                                    value={stat.category}
                                    onChange={(e) => {
                                        const newList = [...statuses];
        ...
        <div className="space-y-12 animate-fade-in pb-20">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
                <div className="flex items-center gap-6">
                    <div className="w-20 h-20 bg-primary-gold/10 rounded-xs flex items-center justify-center text-primary-gold relative group transition-all duration-700 hover:scale-105">
                        <div className="absolute inset-0 bg-primary-gold/20 rounded-xs blur-2xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
                ...
                <div className="flex bg-white/5 p-2 rounded-xs border border-white/5 backdrop-blur-xl shrink-0 h-16">
                    {[
                        ...
                        <button
                            ...
                            className={`px-6 py-2 rounded-xs text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-3 ${activeTab === tab.id ? 'bg-primary-copper text-white shadow-xl shadow-primary-copper/30' : 'text-text-muted hover:text-white hover:bg-white/5'}`}
                        >
            ...
            {message && (
                <div className={`p-6 rounded-xs flex items-center justify-between border animate-slide-up ${message.type === 'success' ? 'bg-green-500/10 border-green-500/20 text-green-500 shadow-[0_0_40px_rgba(34,197,94,0.1)]' : 'bg-red-500/10 border-red-500/20 text-red-500'}`}>
                    <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-xs flex items-center justify-center ${message.type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
